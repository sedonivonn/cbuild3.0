...
export function simulateMatch({ home, away, homeTacticId, awayTacticId, neutral = false }) {
  const A = applyTactic(home, homeTacticId);
  const B = applyTactic(away, awayTacticId);
...
export function simulateKnockout({ home, away, homeTacticId, awayTacticId, twoLeg = true }) {
  if (twoLeg) {
    const leg1 = simulateMatch({ home, away, homeTacticId, awayTacticId });
    const leg2 = simulateMatch({ home: away, away: home, homeTacticId: awayTacticId, awayTacticId: homeTacticId });
    ...
    const et = simulateExtraTime({ home: away, away: home, homeTacticId: awayTacticId, awayTacticId: homeTacticId });
    ...
    const pen = simulatePenalties(home, away);
  ...
  // Single match - Final
  const match = simulateMatch({ home, away, homeTacticId, awayTacticId, neutral: true });
  if (match.home.score !== match.away.score) {
    return { match, winner: match.home.score > match.away.score ? "home" : "away", decidedBy: "regulation" };
  }
  const et = simulateExtraTime({ home, away, homeTacticId, awayTacticId });
...
function simulateExtraTime({ home, away, homeTacticId, awayTacticId }) {
...
function simulatePenalties(home, away) {
  const akeep = home.keeper, bkeep = away.keeper;
  const aatk = home.attack, batk = away.attack;
  let a = 0, b = 0;
  const shots = [];
  for (let i = 0; i < 5; i++) {
